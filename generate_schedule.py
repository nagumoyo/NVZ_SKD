...
# === CSV decoding helpers (Windows/CP932 フォールバック対応・正式版) ==========
import io, os


def _decode_csv_bytes(data: bytes) -> tuple[str, str]:
    candidates = ["utf-8-sig", "utf-8", "cp932", "shift_jis", "utf-16", "latin1"]
    last_err = None
...
def _read_schedule_text(schedule_file) -> tuple[str, str]:
...
def write_to_excel(
    ...
    # ── 生テキスト取得 ────────────────────────────────────────
    raw_text, _csv_encoding = _read_schedule_text(schedule_file)
    # ────────────────────────────────────────────────────────
