...
emp_df["full_name"] = emp_df["col_5"].apply(clean_cell) + emp_df["col_7"].apply(clean_cell)
...
def reshape_block(df, h, d, e):
    ...
    date_cols = [i for i, x in enumerate(raw_dates) if re.match(r"^(0?[1-9]|[12][0-9]|3[01])$", x)]
    date_cols = date_cols[:31] + [None] * (31 - len(date_cols))
    dr = [raw_dates[i] if i is not None and i < len(raw_dates) else "" for i in date_cols]
    rows = df.iloc[d + 1 : e]
    raw_cells = [[clean_cell(row[i]) if i is not None else "" for i in date_cols] for _, row in rows.iterrows()]
...
for h, d, e in blocks:
    ...
    hdr31[1] = ""
    merged_sched = ["\n".join([v for v in sched_df.iloc[:, i] if v]) for i in range(sched_df.shape[1])]
    raw_cols = []
    for idx_col in range(len(date_cols)):
        col_vals = [row[idx_col] for row in raw_cells if idx_col < len(row) and row[idx_col]]
        raw_cols.append(col_vals)
    records.append({"sort": sort_key, "hdr": hdr31, "dr": dr, "sched": merged_sched, "raw_cols": raw_cols, "onb": []})
    ...
    for i, code_str in enumerate(rec["sched"]):
        ...
        for other in records:
            if other is rec:
                continue
            for c in codes:
                if c in other["raw_cols"][i] and other["hdr"][0] not in [n.rstrip('*') for n in names]:
                    name = other["hdr"][0]
...
pd.DataFrame(out).to_csv(os.path.join(BASE_DIR, "blocks_output.csv"), header=False, index=False)
...
def write_to_excel(records, output_path):
    ...
    highlight_fill = PatternFill(fill_type="solid", fgColor="FFFF00")  # 黄色でハイライト
