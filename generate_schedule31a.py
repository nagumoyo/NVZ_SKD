...
def write_to_excel(
    ...
    for rec in records:
        # ── ここから追加 ──
        # 可変長ブロックの高さ = ヘッダー行(1) + 日付行(1) + スケジュール行(1) + 同乗者行数
        onb_count = len(rec.get("onb", []))
        block_height = 3 + onb_count
        # A列に emp_no-行番号 を書き込む
        emp_id = rec["emp_no"]
        ...
        for col, val in enumerate(rec["hdr"], start=1):
            cell = ws.cell(row=current_row, column=col + offset, value=val)
        ...
        for col, dv in enumerate(rec["dr"], start=1):
            cell = ws.cell(row=current_row + 1, column=col + offset, value=dv)
            cell.alignment = Alignment(
        ...
        for col, sv in enumerate(rec["sched"], start=1):
            cell = ws.cell(row=current_row + 2, column=col + offset, value=sv)
            cell.alignment = Alignment(
                horizontal="left", vertical="top", wrap_text=True
            )
