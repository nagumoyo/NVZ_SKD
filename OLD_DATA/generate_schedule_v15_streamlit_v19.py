...
st.title("整形済スケジュール v19e Webアプリ版（同乗行 重複なし）")
...
output_filename = st.text_input("出力ファイル名（例：整形スケジュール_v19e.xlsx）", "整形スケジュール_v19e.xlsx")
    ...
    if schedule_file is None or emp_file is None:
        ...
        for crew in all_crew_schedules:
            ...
            for i in range(len(my_sched_row)):
                my_sched = my_sched_row[i].strip()
                my_prefix = re.match(r"^(\d+)", my_sched)
                if my_prefix:
                    ...
                    for other_crew in all_crew_schedules:
                        ...
                        if i < len(other_crew['schedule_row']):
                            other_sched = other_crew['schedule_row'][i].strip()
                            other_prefix = re.match(r"^(\d+)", other_sched)
