...
if st.button("出力"):
    if schedule_file is None or emp_file is None:
        st.error(
            "スケジュールファイルと職員情報ファイルの両方をアップロードしてください。"
        )
    else:
        emp_df = pd.read_csv(
        ...
        for block in crew_blocks:
            ...
            merged_schedule_row = [""] * 31  # v25b6: 全行JOIN (列数check付)
            for i, col_idx in enumerate(date_col_indices):
                if col_idx >= sched_rows.shape[1]:
                    continue  # 列が足りない場合はskip
                sched_texts = []
                for row_idx in range(sched_rows.shape[0]):
                    cell_value = str(sched_rows.iloc[row_idx, col_idx]).strip()
                    if cell_value != "":
                        sched_texts.append(cell_value)
