...
if st.button("出力"):
    if schedule_file is None or emp_file is None:
        ...
        def is_OB_row(row):
            ...
            if re.match(r"^/s+[A-Z]{3,}OB$", first_cell):
                return True
            for cell in row:
        ...
        for idx, header_idx in enumerate(header_indices_valid):
            ...
            for j in range(header_idx + 2, schedule_df.shape[0]):
                ...
                # v25b8: block_end は OB行 なら break, 通常は次header_idx-1 まで continuation

                if is_OB_row(row):  # OB行の時だけ block_end を決定して break
                    block_end = j - 1
                    break
                    break
            ...
            if is_OB_row(header_row):
                continue
            block_end = next_header_idx - 1  # v25b8: デフォルトは次header直前まで
