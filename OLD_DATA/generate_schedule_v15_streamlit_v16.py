...
st.title("整形済スケジュール v16 Webアプリ版（罫線付き）")
...
output_filename = st.text_input("出力ファイル名（例：整形スケジュール_v16.xlsx）", "整形スケジュール_v16.xlsx")
...
if st.button("出力"):
    if schedule_file is None or emp_file is None:
        ...
        with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
            ...
            for row_idx in range(1, output_df.shape[0]+1, 3):
