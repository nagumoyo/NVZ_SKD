...
def write_to_excel(
    ...
    """
    スケジュールCSVとレコード情報から既存テンプレートExcelに出力する（Ver32a）

    - 第1行のYYYYMMとFLEETを抽出してF1,I1に設定
    - テンプレート構造を保持し、セル上書きのみ実施
    - 1行目の高さ、A列非表示、B〜E列幅を調整
    """
    ...
    # ⑥ 保存
    wb.save(out_xlsx)
    # print("[DEBUG] Workbook formatted and saved")
...
def run(
    schedule_file: Union[str, os.PathLike, bytes, io.BytesIO] = SCHEDULE_FILE,
    pref_file: Union[str, os.PathLike, bytes, io.BytesIO] = PREF_FILE,
    sim_file: Union[str, os.PathLike, bytes, io.BytesIO] = SIM_FILE,
) -> tuple[str, str]:
    """
    schedule_file, pref_file, sim_file は
    - Streamlit 上传時は UploadedFile (BytesIO) を渡す
    - ローカル実行時は何も渡さない（デフォルト定数を使う）
    """
