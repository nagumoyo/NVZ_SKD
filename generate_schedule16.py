...
def write_to_excel(records, out_xlsx):
    ...
    row_num = 1
    for rec in records:
        # header row: disable wrap_text for phone-number fields only
...
def run(schedule_file, emp_file):
    ...
    # affiliation map: use column 1 of emp_no.csv for 所属
    emp_aff_map = {row[2]: row[1] for _, row in emp_df.iterrows()}
    df = sched.copy().map(clean_cell).pipe(remove_blank_and_ob)
    blocks_raw = slice_blocks(df)
    global_dates = blocks_raw[0][3] if blocks_raw else []
    ...
    for h, d, end, dates in blocks:
        ...
        # store employee number and affiliation
        rec_emp_no = code
        ...
        # place affiliation in 31st column (index 30)
        hdr[30] = rec_aff
        new_hdr = []
        for hval in hdr:
            hmod = re.sub(r"PE有効期限", "PE", hval)
            hmod = re.sub(r"PE([0-9]{6})", r"\1", hmod)
            hmod = re.sub(r"社員番号", "職番", hmod)
            hmod = re.sub(r"電話番号", "電話", hmod)
            new_hdr.append(hmod)
        hdr = new_hdr
    ...
    for rec in records:
        ...
        for idx in range(len(global_dates)):
            flights = [e for e in rec["full_entries"][idx] if re.match(r"^[0-9]", e)]
            names = []
            for other in records:
                if other is rec:
                    continue
                other_flights = [
                    e for e in other["full_entries"][idx] if re.match(r"^[0-9]", e)
                ]
...
def run(schedule_file, emp_file):
    ...
    emp_name_map = {row[2]: row[4] for _, row in emp_df.iterrows()}
    emp_two_map = {row[2]: row[6] for _, row in emp_df.iterrows()}
    # affiliation map: use column 0 of emp_no.csv for 所属
