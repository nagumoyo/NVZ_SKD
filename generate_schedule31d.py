...
def write_to_excel(
    ...
    """
    全レコードを Excel ファイルに書き出す（Ver31c C/D列追加版）。

    - 先頭に5列追加 → A列: emp_no-行番号, B列: 英字ヘッダー名+2レター
    - C列: ヘッダー行の AI 列相当 (rec['hdr'][29])
    - D列: ヘッダー行の AJ 列相当 (rec['hdr'][30])
    - 1行目にフィルターヘッダーを配置（A～E列）
    - データは2行目以降に書き出し
    - ヘッダー行に罫線／日付行に背景色／スケジュール行に折返し
    - 同乗者行は write_onboard_rows で可変長処理
    """
    wb = Workbook()
    ws = wb.active
    # シート名を Schedule に変更
    ...
    for rec in records:
        ...
        for i in range(block_height):
            ws.cell(row=current_row + i, column=1, value=f"{emp_id}-{i+1}")  # A列
            ws.cell(row=current_row + i, column=2, value=full_name)  # B列
            ws.cell(row=current_row + i, column=3, value=ai_val)  # C列: AI
            ws.cell(row=current_row + i, column=4, value=aj_val)  # D列: AJ
